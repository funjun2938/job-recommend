...
import {
  CheckCircle2, ArrowRight, ArrowDown, Sparkles, Users, GitBranch,
  Briefcase, ChevronDown, ChevronUp, ShieldCheck,
} from 'lucide-react'
import { providerMeta, toStage1, type UnifiedProfile } from '@/lib/connect'
...
export function ConnectedProfile() {
  ...
  const meta = providerMeta(profile.source)
    ...
    <div className="min-h-screen bg-gray-50 pb-28">
      ...
      <div className="bg-white border-b border-gray-100 px-5 pt-10 pb-6">
        <div className="flex items-center gap-2 mb-4">
          <span className="w-7 h-7 rounded-lg flex items-center justify-center text-white text-xs font-bold" style={{ background: meta.brand }}>
            {meta.name[0]}
          </span>
          <span className="flex items-center gap-1 text-sm font-bold text-emerald-600">
            <CheckCircle2 size={15} /> {meta.name} 연동 완료
          </span>
        </div>
        ...
        <div className="mt-4 flex items-center gap-3">
          <div className="flex-1 h-2 bg-gray-100 rounded-full overflow-hidden">
            <div className="h-full bg-indigo-500 rounded-full" style={{ width: `${profile.completeness}%` }} />
          </div>
          <span className="text-xs font-bold text-indigo-600">프로필 완성도 {profile.completeness}%</span>
        </div>
      </div>
      ...
      <div className="px-5 py-5 space-y-4">
        ...
        <div className="flex items-center gap-2 text-[11px] text-gray-500 bg-white border border-gray-100 rounded-xl px-3 py-2">
          ...
          <span>어느 서비스로 연동하든 <b className="text-gray-700">동일한 표준 규격</b>으로 정규화돼요</span>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-5 space-y-4">
          ...
          {/* 스킬 */}
          <div>
            <p className="text-[10px] text-gray-400 mb-1.5">핵심 스킬</p>
          ...
          {profile.networkReach && (
            <div className="flex items-center gap-2 bg-violet-50 rounded-xl px-3 py-2.5">
              ...
              <p className="text-xs text-violet-700">
                인맥 <b>{profile.networkReach.connections.toLocaleString()}명</b> · 연결 회사 <b>{profile.networkReach.companies}곳</b> 확인
              </p>
            </div>
          )}
        </div>

        {/* 정규화 매핑 — 우리 규격으로 어떻게 변환됐는지 */}
        <div className="bg-white rounded-2xl border border-gray-100 p-5 space-y-3">
          <div className="flex items-center gap-2">
            <Sparkles size={16} className="text-amber-500" />
            <h2 className="font-bold text-gray-900 text-sm">{meta.name} 데이터 → 표준 규격 변환</h2>
          </div>
          <p className="text-[11px] text-gray-400 -mt-1">가져온 원천 데이터를 우리 표준 필드로 매핑했어요</p>

          <div className="space-y-2">
            {profile.mappings.map((m, i) => (
              <div key={i} className="rounded-xl border border-gray-100 p-3">
                <div className="flex items-center gap-2 text-xs">
                  <span className="font-mono text-[11px] text-gray-400">{m.rawField}</span>
                  <span className="text-gray-700 truncate">{m.rawValue}</span>
                </div>
                <div className="flex items-center gap-1.5 mt-1.5">
                  <ArrowDown size={12} className="text-indigo-400" />
                  <span className="text-[10px] font-bold text-indigo-500 bg-indigo-50 px-1.5 py-0.5 rounded">{m.field}</span>
                  <span className="text-xs font-semibold text-gray-800">{m.normalized}</span>
                </div>
              </div>
            ))}
          </div>
          ...
          {/* 원천 신호 */}
          <div className="pt-1">
            <p className="text-[10px] text-gray-400 mb-1.5">읽어온 원천 데이터</p>
          ...
          <button
            ...
            className="flex items-center gap-1 text-[11px] font-semibold text-gray-400 pt-1"
          >
            {showJson ? <ChevronUp size={13} /> : <ChevronDown size={13} />} 표준 규격 원본(JSON) {showJson ? '접기' : '보기'}
          </button>
          {showJson && (
            <pre className="text-[10px] leading-relaxed bg-gray-900 text-gray-100 rounded-xl p-3 overflow-x-auto">
{JSON.stringify({ spec: profile.spec, source: profile.source, canonical: c, networkReach: profile.networkReach }, null, 2)}
            </pre>
          )}
        </div>
      </div>
      ...
      <div className="fixed bottom-0 inset-x-0 bg-white/90 backdrop-blur border-t border-gray-100">
        <div className="max-w-[430px] mx-auto px-5 py-3 space-y-2">
          ...
          <Link href="/" className="block text-center text-xs text-gray-400">다른 계정으로 다시 연동</Link>
        </div>
      </div>
    </div>
  )
